...
make_MEs <- function(fix_effs, ran_effs_by_K, mix_eff_ind_pairs){
  ...
  ME_inds_in_REs = purrr::map_vec(mix_eff_ind_pairs, 2)

  ## Construct list of vectors containing random-only effects ----
  K = length(ran_effs_by_K)
  ran_only_effs_by_K = ran_effs_by_K
  for(k in 1:K){
    # Remove random effects which have been used to construct mixed effects ----
    ran_only_effs_by_K[[k]] = purrr::discard_at(ran_effs_by_K[[k]], ME_inds_in_REs)
  }
